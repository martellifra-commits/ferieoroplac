...
st.set_page_config(
    page_title="Calendario Ferie - OROPLAC", layout="wide"
)
...
# Style CSS Custom per tabelle e pulsanti
st.markdown(
    """
    <style>
    .stButton > button {
        ...
        for day_idx, day in enumerate(week):
            with cols[day_idx]:
                if day == 0:
                    ...
                    # Pulsante per ogni giorno
                    btn_label = f"{day}\n"
                    if st.button(
                        f"**{day}**", key=f"btn_{date_key}", help=f"Data: {date_key}"
                    ):
                        # Logica di attivazione/disattivazione ferie
                    ...
                    # Mostra i badge sotto il giorno
                    if badges_html:
                        st.markdown(
